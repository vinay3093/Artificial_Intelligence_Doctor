...
def initialization_params(options):
	...
	if len(embFile) > 0:
		...
		params['W_emb'] = add_embed_layer(embFile)
...
def setup_model(tparams, options, W_emb=None):
...
def load_files(seqFile, labelFile, timeFile, topicFile):
...
def crossentropy_calc(test_model, dataset, options):
	inputDimSize = options['inputDimSize']
	numClass = options['numClass']
...
def train_model(
	...
	params = initialization_params(options)
	...
	if useTime and embFineTune:
		...
		use_noise, x, y, t, topics, mask, lengths, cost =  setup_model(tparams, options)
		...
		use_noise, x, y, t, topics, mask, lengths, cost =  setup_model(tparams, options, W_emb)
		grads = T.grad(cost, wrt=tparams.values())
		f_grad_shared, f_update = adadelta(tparams, grads, x, y, mask, lengths, cost, options, t, topics)
	elif not useTime and embFineTune:
		print('not using duration information, fine-tuning code representations')
		use_noise, x, y, topics, mask, lengths, cost =  setup_model(tparams, options)
		grads = T.grad(cost, wrt=tparams.values())
		...
		W_emb = theano.shared(params['W_emb'], name='W_emb')
		use_noise, x, y, topics, mask, lengths, cost =  setup_model(tparams, options, W_emb)
		grads = T.grad(cost, wrt=tparams.values())
		f_grad_shared, f_update = adadelta(tparams, grads, x, y, mask, lengths, cost, options, None, topics)

	print('Loading data ... ')
	trainSet, validSet, testSet = load_files(seqFile, labelFile, timeFile,topicFile)
	...
	for epoch in xrange(max_epochs):
		...
		validAuc = crossentropy_calc(test_model, validSet, options)
		...
		if validAuc < bestValidCrossEntropy:
			...
			bestParams = extract_zip(tparams)
			testCrossEntropy = crossentropy_calc(test_model, testSet, options)
			print('Test cross entropy:%f at epoch:%d' % (testCrossEntropy, epoch))
			tempParams = extract_zip(tparams)
